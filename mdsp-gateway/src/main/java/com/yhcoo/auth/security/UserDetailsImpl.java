package com.yhcoo.auth.security;

import com.yhcoo.common.enums.UserStatusEnum;
import com.yhcoo.common.vo.SysRoleVo;
 ...
 * 本地测试，几个系统用的同一个redis，对象在一个系统中序列化存入redis，包路径com.yhcoo.auth.security.UserDetailsImpl；
 * 主要原因是因为key也是相同，在运行另一个系统时，发现这个key已经存在了，取出时，如果网关这个系统不存在com.yhcoo.auth.security.UserDetailsImpl这个类的话
 * 会报错redis序列化问题，com.yhcoo.auth.security.UserDetailsImpl找不到，并且前端会报跨域错误。
